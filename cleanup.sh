...
if [ -e $HOME/netbricks_logs ]; then
	...
	cd $HOME/logs && tar -cvzf netbricks_logs--$current_date_time.tar.gz  netbricks_logs--$current_date_time
...
if ls $HOME/dev/pvn/utils/*.log >/dev/null 2>&1; then
	...
	cd $HOME/logs && tar -cvzf pktgen_logs--$current_date_time.tar.gz  pktgen_logs--$current_date_time
