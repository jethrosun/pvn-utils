...
if [ ! -e /data/bt/config/auth ]; then
	...
	deluge-console -c /data/bt/config 'config -s max_active_limit 10'
	deluge-console -c /data/bt/config 'config -s max_active_downloading 10'

	deluge-console -c /data/bt/config 'config -s max_active_seeding 10'
	deluge-console -c /data/bt/config 'config -s max_download_speed_per_torrent 2000'
	deluge-console -c /data/bt/config 'config -s max_upload_speed_per_torrent 2000'
	# deluge-console -c /home/jethros/bt_data/config 'config -s max_seed_speed_per_torrent 2500'

	deluge-console -c /data/bt/config "halt"
fi

echo "Starting up now ..."
nice deluged -c /data/bt/config
