...
# The following two p2p experiments are deprecated for now as we don't want to
# run uncontrolled expr.
"""
    ...
    for trace in mix.traces:
        ...
        for setup in mix.p2p_controlled_list:
            ...
            for epoch in range(mix.num_of_epoch):
                netbricks_sess = netbricks_sess_setup(trace, nf, epoch, expr)

                leecher1_sess = p2p_sess_setup('flynn', trace, nf, epoch)
                leecher2_sess = p2p_sess_setup('tao', trace, nf, epoch)
                leecher3_sess = p2p_sess_setup('sanchez', trace, nf, epoch)
                ...
                run_p2p_node('leecher', leecher2_sess, setup, epoch)
                run_p2p_node('leecher', leecher3_sess, setup, epoch)
                run_netbricks(netbricks_sess, trace, nf, epoch, setup, expr)
...
main(mix.p2p_controlled)
# main(p2p_ext, 5)
print("All experiment finished {}".format(mix.p2p_controlled))
